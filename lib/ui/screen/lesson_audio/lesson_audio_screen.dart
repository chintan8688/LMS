...
class LessonAudioScreenArgs {
  ...
  LessonAudioScreenArgs(this.courseId, this.lessonId, this.authorAva,
      this.authorName, this.hasPreview, this.trial);
}

class LessonAudioScreen extends StatelessWidget {
  static const routeName = 'lessonAudioScreen';
  final LessonAudioBloc _bloc;

  const LessonAudioScreen(this._bloc) : super();

  @override
  Widget build(BuildContext context) {
    final LessonAudioScreenArgs args =
        ModalRoute.of(context).settings.arguments;

    return BlocProvider<LessonAudioBloc>(
        create: (c) => _bloc,
        child: _LessonAudioScreenWidget(args.courseId, args.lessonId,
            args.authorAva, args.authorName, args.hasPreview, args.trial));
  }
}

class _LessonAudioScreenWidget extends StatefulWidget {
  final int courseId;
  final int lessonId;
  final String authorAva;
  final String authorName;
  ...
  const _LessonAudioScreenWidget(this.courseId, this.lessonId, this.authorAva,
      this.authorName, this.hasPreview, this.trial);

  @override
  State<StatefulWidget> createState() {
    return _LessonAudioScreenState();
  }
}

class _LessonAudioScreenState extends State<_LessonAudioScreenWidget> {
  LessonAudioBloc _bloc;
  ...
  void initState() {
    super.initState();
    _bloc = BlocProvider.of<LessonAudioBloc>(context)
  ...
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<LessonAudioBloc, LessonAudioState>(
      bloc: _bloc,
  ...
  _buildTitle(state) {
    if (state is InitialLessonAudioState) {
      return Center();
    }

    if (state is LoadedLessonAudioState) {
      return Row(
  ...
  _buildBottom(LessonAudioState state) {
    if (state is InitialLessonAudioState) {
      return Center(
        child: CircularProgressIndicator(),
      );
  ...
  _buildButtonChild(LessonAudioState state) {
    if (state is InitialLessonAudioState)
      return SizedBox(
        width: 30,
        height: 30,
