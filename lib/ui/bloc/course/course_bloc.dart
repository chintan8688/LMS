...
class CourseBloc extends Bloc<CourseEvent, CourseState> {
  ...
  Stream<CourseState> mapEventToState(
    ...
    if (event is RequestForBookEvent) {
      yield* _mapBookRequestEventToState(event);
    }
  ...
  Stream<CourseState> _fetchCourse(courseId) async* {
    ...
    try {
      ...
      yield LoadedCourseState(courseDetailResponse);
    } catch (e, s) {
      print(e);
      print(s);
  ...
  Stream<CourseState> _mapBookRequestEventToState(event) async* {
    try {
      await _coursesRepository.bookRequest(event.courseId, event.notes);
    } catch (error) {
      print(error);
    }
  }
